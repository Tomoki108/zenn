...
- **Consistent Hashing**は、サーバー台数の増減時の再配置が 平均 `numOfKey / numOfServer`で済むテクニック。
  - ハッシュ化されたキーが取りうる値のリストの、先頭と末尾がつながったデータ構造を用意する。
  - 各サーバーについて、IP や名前をもとにハッシュを作成し、データ構造の該当する位置に目印をつける。
  - 保存したいデータが来たら、ハッシュキーの位置から時計回りでデータ構造を探索し、最初に見つけたサーバーに格納することにする。
