---
title: "AtCoder Beginer Contest 373~382 Dまでの要点・解答（Go）"
emoji: "🦁"
...
## [D - Cycle](https://atcoder.jp/contests/abc376/tasks/abc376_d) / BFS（グラフ）
...
## [D - Hidden Weights](https://atcoder.jp/contests/abc373/tasks/abc373_d) / BFS or DFS（グラフ）

- ある２点とその辺について、辺の向きに関わらず一方の点の値が分かっていれば他方の点の値が分かる。

  - $toX - fromX = weight$ から $fromX - toX = -weight$ が成り立つため。
  - `var weights map[int][][2]int`のように重みを記録し、from => to だけではなく to => from の重みも記録する。**有効辺グラフであることに囚われすぎない。**
...
- あるいは、M に対して 3 の 10 乗（制約よりマックスの乗数）を可能な限り引く、3 の 9 乗を可能な限り引く...という様にして解いても良い。
