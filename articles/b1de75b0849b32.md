...
- 該当範囲を重複してカウントしないための工夫が必要。
  - 全探索なら、二つのうちいずれかの加湿器からの**マンハッタン距離**以内かを同時に判定すればいい。
...
- max で、グリッドサイズが 1000\*1000、移動距離が 1000、始点ノードの数も 1000 なので、全探索は間に合わない。
...
- ある始点ノードからの探索において別の始点ノードに到達した場合、そこから先は探索しなくていい。その視点からの BFS に内包されている範囲だから。
