...
- 友達の友達も友達にする。これをできなくなるまで繰り返す。これはグラフの連結成分について、**完全グラフ**（全ての頂点から他の全ての頂点への辺が存在する）にするということ。

- **完全グラフの辺の数は $\binom{N}{2}$**

- **Union Find**を用いて、各ノードを連結する。ルートノード毎のサイズから「各**連結成分**（辺で行き来出来る範囲）が完全グラフになった時の辺数」の和を求めていく。最初に存在していた辺数 M を引けば、答えになる。
...
## [E - Maximize Rating](https://atcoder.jp/contests/abc327/tasks/abc327_e) / シグマ、DP(テーブル)、逆から見る

- レートの式は、$(0.9)^{k-i} * Q_i$ の項を除いて定数。
- 全ての k で `dp[i][j] // i番目まで処理しj個選んでいる時の、最大の合計`という **DP** をやれば解けそうだが、計算量が大体 $O(N^3)$ となり間に合わない。
- k 毎に DP をしたいのは、$(0.9)^{k-i}$ の指数が k によって変わるから。**逆に、index N-1 から 0 の順で`Ps`を見ていき**、最後の一つから順に取るか取らないかを判定する。そうすると指数は`0, 1, ...`となり k によらなくなる。各`dp[N][j]`が、`k ＝ j` の時に最適な時に選択をした時の合計になる。それらから定数を差し引きしたもので、`maxAns` の更新を試みる。
- 0.9 の乗数や各定数は予め作ってスライスに入れて後で再利用する。

:::details code
https://github.com/Tomoki108/go-algo/blob/main/ABC/abc327/e/main.go
:::
...
- `2*N`のサイズの `graph` を用意し、`i => i+N (cost: 0)`の辺、`i+N <=> j+N (cost: Dss[i][j]) `の辺を追加してダイクストラをやり、`ans = min(dists[N-1], dists[2*N-1])`。
...
- `S`を前から順に見て、それより前の`S'`に、結合して条件を満たすものがいくつあるかを高速に求められたら嬉しい。
...
- `dp := make([]int, X)// dp[i]: 時刻iに丁度曲が終了する確率`の dp をやればいい。
  - 0~X の時間でループ、その中で全曲分ループして遷移（X を超える時間は無視）。
...
## [E - Defect-free Squares](https://atcoder.jp/contests/abc311/tasks/abc311_e) / 二次元累積和、包除原理、二分探索

- 穴の空いているマスが 1, 空いていないマスが 0 のグリッドを用意し、そこから**二次元累積和**を作成する。
- 全ての n の全ての四角に対して**包除原理**で穴が 0 個かを判定すると間に合わない。
- 左上を固定し、n をどこまで小さくすると穴がゼロになるのかを**二分探索**し、`ans += n`する。（四角の大きさを小さくするほど穴が少なくなる単調増加性がある。）

:::details code
https://github.com/Tomoki108/go-algo/blob/main/ABC/abc311/e/main.go
:::
...
## [E - NAND repeatedly](https://atcoder.jp/contests/abc310/tasks/abc310_e) / NAND

- **NAND(否定論理積)**は、片方でも偽なら偽、そうでないなら真。
  - **NAND には結合法則も交換法則も成り立たない。累積 NAND も作れない。**
- **数列の連続部分区間は、各要素を末尾とする連続部分列を列挙すると全列挙できる。**
- 数列を前から見て、その要素を末尾とする連続部分列の演算結果を考える。
  - `S[i] == 0`:
    - `0`になる部分列: 1（その要素のみの連続部分列の場合）
    - `1`になる部分列: `S[i-1]`を末尾とする連続部分列に、結果が 0 or 1 になるものがいくつあるか
  - `S[i] == 1`
    - `0`になる部分列: `S[i-1]`を末尾とする連続部分列に、結果が 0 になるものがいくつあるか
    - `1`になる部分列: `S[i-1]`を末尾とする連続部分列に、結果が 1 になるものがいくつあるか + 1（その要素のみの連続部分列の場合）
- よって、zeroCnt, oneCnt を更新しながら前から見ていけばいい。

:::details code
https://github.com/Tomoki108/go-algo/blob/main/ABC/abc310/e/main.go
:::
...
## [E - Distinct Adjacent](https://atcoder.jp/contests/abc307/tasks/abc307_e) / 円環、DP(テーブル)

- 円環になっていなければ、$ans = M*(M-1)^{N-1}$ だが最後の人が最初の人と同じ数字の場合を考慮できていない。
- `dp[i][j] // i: index i人目まで確定、j: 0 or 1, 最初の人と同じ数字かどうか`の**DP**をし、`dp[N-1][0]`が答え。

:::details code
https://github.com/Tomoki108/go-algo/blob/main/ABC/abc307/e/main.go
:::
...
- prevVal が rest に含まれるなら rest から削除(1)、そうでないなら topK から削除。(2)（両方に存在するときは rest から削除して良い。）
